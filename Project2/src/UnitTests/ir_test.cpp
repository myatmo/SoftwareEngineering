...
TEST_F(IRTest, InvalidatedVotes1){
    // to check the invalidated votes and winner
    IR* ir = new IR();
    ir->setNumBallots(10);
    ...
    vector<Ballot*> expected = {};
    for(int i=0; i<(int)votes_case1.size();i++){
        Ballot* ball = new Ballot();
        ball->setRank(1);
        ball->setVote(votes_case1[i]);
        ball->setID(i);
        ball->checkInvalidBallot();
        if (ball->isInvalidated()) {
            ir->pushInvalidatedBallots(ball);
        }
        if (i == 4) {
            expected.push_back(ball);
        }
    ...
    vector<Ballot*> actual = ir->getInvalidatedBallots();
    EXPECT_EQ(actual,expected);
    EXPECT_EQ(ir->getWinner(),ir->getCandidateList()[0]);
    delete ir;
}

TEST_F(IRTest, InvalidatedVotes2){
    // to check multiple invalidated votes
    IR* ir = new IR();
    ir->setNumBallots(10);
	ir->setNumCandidates(4);
    vector<string> votes_case2 = {",,1,", "1,3,2,", "1,,,", "1,,2,", ",,1,", "1,2,,", ",1,2,3", ",1,,2", ",,1,2", ",2,,1"};
    for (int i = 0; i < ir->getNumCandidates(); i++) {
        Candidate* cand = new Candidate();
        cand->setName("Cand"+to_string(i));
        cand->setNumVotes(0);
