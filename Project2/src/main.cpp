...
void loadFile(string votingType, OPL* opl, IR* ir, string fn) {
    ...
    //start infinite while loop for csv file entry, will only be broken when "done" is entered
    while(1){
        //initializw a boolean each loop to start with no error
        bool error = false;
        string fileName, firstLine;
        std::ifstream file;
        int numBall = 0;
        printf("Please enter the correct file name or done to continue to vote counting:\n");
        cin>>fileName;
        // if the user input is done, break the inf loop
        if(fileName == "done"){
            break;
        }
        // open the entered file name
        string path = "../testing/";
        ...
        while (error) {
            ...
            printf("Please enter the correct file name or done to continue to vote counting:\n");
            cin>>fileName;
            // if user enters another input by mistake they can type "done" to get out of this loop
            if(fileName == "done"){
                break;
            }
            //check if the newly entered file exists and is the correct type
            path = "../testing/";
            path.append(fileName);
            file.open(path.c_str(), std::ifstream::in);
            getline(file, firstLine);
        ...
        // if the user entered "done" in the previous loop, break out of this inf loop as well
        if(fileName == "done"){
            break;
        }
        ...
        // count will control data input and starts as 1, will increment each loop of the inf loop
        if(votingType == "IR"){
            string filename = "./Audits/IR"+fn;
            ofstream audit;
            audit.open(filename, std::ios_base::app);
            // numCand only needs to be set once
            if(count == 1){
            ...
            for (int i=0; i<numCand-1; i++){
                // can only set candidates once to keep data from previous csv reads
                if(count == 1){
                    string candidate;
                ...
                // if count > 1 then we need to read the lines but not store that data in our class attributes
                else{
                    string candidate;
                    getline(file, candidate,' ');
                    string party;
            ...
            // if count > 1, then we need to read that data to advance the file pointer
            else{
                string candidate;
                getline(file, candidate,' ');
                string party;
            ...
            // always need to determine number of ballots in current file for the ballot processing loop control
            ...
            for(int i=0; i<numBall; i++){
                ...
                ballot->setRank(1);
                ballot->setID(i);
              	ballot->checkInvalidBallot();	// check whether the ballot is invalid or not
			    audit<<"checking invalidated: "<<ballot->isInvalidated()<<endl;
                if(!ballot->isInvalidated()){
                    ...
                    ir->pushInvalidatedBallots(ballot);
                    audit<<"Process ballot "<<to_string(i)<<": "<<vote<<" given to nobody\n";
                    invBall++;
                }

            }
            ir->setNumBallots(numBall - invBall);
            audit.close();
        }
        // this is for OPL voting type
            ...
            ofstream audit;
            audit.open(filename, std::ios_base::app);
            // only need to set the number of candidates once
            if(count == 1){
            ...
            for (int i=0; i<numCand-1; i++){
                // only set candidates for the first file that is input
                if(count == 1){
                    string noneed;
                    string candidate;
                    getline(file, noneed,'[');
                ...
                // else advacne the pointer without setting up new candidates
                else{
                    string noneed;
                    string candidate;
                    getline(file, noneed,'[');
            ...
            // only if on the first file in the loop
            if(count == 1){
                string noneed;
                string candidate;
                getline(file, noneed, '[');
            ...
            // else read the info to advance the file pointer
            // candidates info already set in first loop
            else{
                string noneed;
                string candidate;
                getline(file, noneed, '[');
            ...
            // alwasy needs to be done for the ballot processing loop control
            getline(file, numBallots, '\n');
            stringstream bal(numBallots);
            bal>>numBall;
        ...
        // this is for PO election type
        else if(votingType == "PO"){
            string filename = "./Audits/PO"+fn;
            ofstream audit;
            audit.open(filename, std::ios_base::app);
            // numCand only needs to be set once
            if(count == 1){
                audit<<"PO Election Run Audit\n";
                audit<<"PO voting info colleciton\n";
                audit<<"Starting read from file\n";
                ir->setNumCandidates(numCand);
                audit<<"Number of candidates set to: "<<to_string(numCand)<<endl;
            }
        ...
        // close the open file
        file.close();
        // increment coutn for the next loop
        count++;    
    }
    // once outside the loop, we know that all candidates have their votes for OPL voting and can add them to their party lists
    if(votingType == "OPL"){
...
void initializeVotingSystem(string fn, string invalid_ballot_file) {
    /*
    ...
    */
    ...
    bool error = true;
    while (error) {
        // prompt user to enter 1 for 'IR' voting, 2 for 'OPL' voting, or 3 for 'PO' voting
        ...
        printf("Please choose 3 to select Popularity Only voting system (PO)\n    NOTE: This will only process the input CSV file(s), \n         it will not count the votes and determine a winner\n");
        ...
        // check if user input is either 1 , 2 , or 3
        if (user_choice == 1 || user_choice == 2 || user_choice == 3) {
            error = false;
        }
        // error message if the input is not either 1 , 2, or 3 otherwise continue
        else {
    ...
    string votingType;
    // set Election::setvotingType to the user choice (if user input is 1, set it to 'IR', if 2 set to 'OPL', else set to 'PO')
    if (user_choice == 1) {
        votingType = "IR";
    }
    if(user_choice == 2) { 
    ...
    if (votingType == "IR"){
        ...
        string inv_file = "./Audits/"+invalid_ballot_file;
        ofstream audit;
        audit.open(inv_file, std::ios_base::app);
        printf("Saving Invalidated Ballots to a file...\n");
        for(int i = 0; i < (int)ir->getInvalidatedBallots().size(); i++) {
          audit << *ir->getInvalidatedBallots()[i] << endl;
        }
    }
    else if (votingType == "OPL") {
    ...
    // no count vote implementation yet for PO so print message to confirm csv file was parsed
    else{
        printf("Voting Type is PO, the ballots will not be counted but have been processed\n");
        printf("Check the Audit file to confirm this has been done correctly\n");
    }
    // only display result if voting type is not PO
    if(votingType != "PO"){
...
int main(int argc, char *argv[]) {
    ...
    string invalid_ballot_file = "invalidated_"+string(buffer);
    srand(time(NULL));	// to get true random number each time
    printf("Welcome to Voting System Application\n");
    initializeVotingSystem(fn, invalid_ballot_file);
